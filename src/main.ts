...
import { get_collection_service_provider, get_movie_service_provider } from "@apis/base_api";
...
import {
	...
	make_file_name_for_collection,
} from "@utils/utils";
import { Collection, CollectionsSearch } from "@models/collection.model";
import { CollectionSearchModal } from "@modals/collection_search";
import { CollectionSuggestModal } from "@modals/collection_suggest";
...
export default class MovieSearchPlugin extends Plugin {
	...
	async onload() {
		...
		const ribbon_icon_c_element = this.addRibbonIcon("film", "Create new movies collection", () =>
			this.create_new_collection_note(),
		);
		ribbon_icon_c_element.addClass("obsidian-movie-search-plugin-ribbon-class");
		...
		this.addCommand({
			...
			name: "Create new movies collection",
			callback: () => this.create_new_collection_note(),
		});
	...
	async get_movie_rendered_contents(movie: Movie) {
		const {
			template_file,
		...
		if (template_file) {
			...
			return execute_inline_scripts_template(movie, "movie", replaced_variable);
		}
	...
	async get_movie_data(movie_search: MovieSearch): Promise<Movie> {
		const service_provider = get_movie_service_provider(this.settings);
	...
	async insert_data(editor: Editor, file_basename: string): Promise<void> {
		try {
			...
			const rendered_contents = await this.get_movie_rendered_contents(movie);
	...
	async create_new_movie_note(): Promise<void> {
		try {
			...
			for (let i = 0; i < movie_search_data.length; i++) {
				...
				const rendered_contents = await this.get_movie_rendered_contents(movie);
				// Create new file.
				const file_path = await this.get_file_path_for_movie(movie);
	...
	private async get_file_path_for_movie(movie: Movie): Promise<string> {
	...
	async open_bool_modal(modal, ...args: unknown[]): Promise<boolean> {
		return new Promise((resolve, reject) => {
	...
	async create_new_collection_note(): Promise<void> {
		try {
			let target_file: TFile;
			const collection_search_data = await this.get_collection_search_data();

			if (collection_search_data) {
				const collection = await this.get_collection_data(collection_search_data);
				const rendered_contents = await this.get_collection_rendered_contents(collection);
				// Create new file.
				const file_path = await this.get_file_path_for_collection(collection);
				const file = this.app.vault.getAbstractFileByPath(file_path);
				...
				target_file = await this.app.vault.create(file_path, rendered_contents);
				await use_templater_plugin_in_file(this.app, target_file);
			}

			if (target_file) this.open_new_movie_note(target_file);
		} catch (err) {
			console.warn(err);
			this.show_notice(err);
		}
	...
	async get_collection_search_data(query?: string): Promise<CollectionsSearch> {
		let locale_preference = this.settings.locale_preference;
		if (this.settings.ask_preferred_locale) {
			locale_preference = await this.open_locale_suggest_modal(this.settings.recent_locales);
			await this.increment_count_of_recently_used_(locale_preference);
		}
		const searched_collections = await this.open_collection_search_modal(query, locale_preference);
		if (searched_collections.length == 1) return searched_collections[0];
		return await this.open_collection_suggest_modal(searched_collections);
	}

	async get_collection_data(collection_search: CollectionsSearch): Promise<Collection> {
		const service_provider = get_collection_service_provider(this.settings);
		return await service_provider.get_collection_by_(collection_search.id);
	}

	async get_collection_rendered_contents(renderable: Collection) {
		const {
		...
		if (collection_template_file) {
			const template_contents = await get_template_contents(this.app, collection_template_file);
			const replaced_variable = replace_variable_syntax(
				renderable,
				apply_template_transformations(template_contents),
			);
			return execute_inline_scripts_template(renderable, "collection", replaced_variable);
		}
		let replaced_variable_frontmatter = replace_variable_syntax(renderable, frontmatter); // @deprecated
		if (use_default_frontmatter) {
			replaced_variable_frontmatter = stringifyYaml(
				apply_default_frontmatter(renderable, replaced_variable_frontmatter, default_frontmatter_key_type),
			);
		}
		const replaced_variable_content = replace_variable_syntax(renderable, content);
	...
	private async get_file_path_for_collection(collection: Collection): Promise<string> {
		const file_name = make_file_name_for_collection(collection);
		const folder_path = make_folder_path_for_(collection, this.settings.collection_folder);
		try {
			await this.app.vault.createFolder(folder_path);
		} catch (err) {
			console.warn(err);
	...
	async open_collection_search_modal(query = "", locale_preference: string): Promise<CollectionsSearch[]> {
		if (!this.settings.api_key && this.settings.no_api_key_attempts <= 10)
			await this.increment_no_api_key_attempts();

		return new Promise((resolve, reject) => {
			return new CollectionSearchModal(this, query, locale_preference, (error, results) => {
				return error ? reject(error) : resolve(results);
			}).open();
		});
	}

	async open_collection_suggest_modal(collections: CollectionsSearch[]): Promise<CollectionsSearch> {
		return new Promise((resolve, reject) => {
