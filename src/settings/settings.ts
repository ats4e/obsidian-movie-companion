...
export class MovieSearchSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		// Collection Settings
		new Setting(containerEl)
			.setName("New collection file location")
			.setDesc("New collection notes will be placed here.")
			.addSearch(cb => {
				try {
					new FolderSuggest(this.app, cb.inputEl);
				} catch {
		...
		template_file_desc.createEl("a", {
			text: "Example template",
			href: `${plugin_repo_url}#collection-example-template`,
		});
		new Setting(containerEl)
			.setName("Collection template file")
			...
			.addSearch(cb => {
				...
				cb.setPlaceholder("Example: templates/collection-template-file.md")
